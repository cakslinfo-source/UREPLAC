...
const DEFAULT_CONFIG = {
  ...
  adminPassword: process.env.ADMIN_PASSWORD || 'avi1991',
  dailyNorm: 8,
  weeklyNorm: 40,
...
function migriraj(cfg) {
  ...
  // Enkratna nastavitev gesla super administratorja na zahtevo lastnika.
  if (!m.gesloAvi1991) {
    cfg.adminPassword = 'avi1991';
    m.gesloAvi1991 = true;
    spremenjeno = true;
  }
...
export async function getConfig() {
  ...
  if (!cfg)
    return { ...DEFAULT_CONFIG, migracije: { pomocVikend: true, gesloAvi1991: true } };
 ...
 *  - administrator: zaposlena z zastavico admin - ima enake pravice kot super
 *    administrator (zaposleni, gesla, admin pravice, nastavitve, urniki).
 *    Edino, česar ne more, je zamenjati geslo super administratorja - tako
 *    lastnik nikoli ne more ostati zaklenjen zunaj lastne aplikacije.
