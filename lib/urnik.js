...
// Vrednosti v "nemorem": 'ves' | 'dop' | 'pop'
...
// nemorem: mapa { date: 'ves'|'dop'|'pop' }, evid: mapa { date: {t:...} }
export function jeProsta(empId, date, sh, nemorem, evidenca) {
...
export function ureVTednu(urnikDays, dni, empId, shifts) {
  let ur = 0;
  let st = 0;
  for (const d of dni) {
...
export function predlagajUrnik({
  ...
  for (const e of employees) {
    stanje[e.id] = { ur: 0, dnevi: new Set(), zapored: 0 };
  }
  ...
  for (const d of dni) {
    for (const sh of shifts) {
      ...
      const kandidati = employees
        ...
        .filter((e) => jeProsta(e.id, d, sh, nemorem, evidenca))
        .map((e) => {
