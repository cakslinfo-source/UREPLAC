...
export function predlagajUrnik({
  ...
  // Zaposlena: polna tedenska norma (lahko svoja).
  // Študentka: približno število ur, ki ga vpiše administrator;
  // če ni vpisano, nima cilja in pride na vrsto za tistimi, ki ga imajo.
  const normaZa = (e) => {
    if (e.kind === 'studentka') {
      return e.weeklyNorm == null || e.weeklyNorm === '' ? null : Number(e.weeklyNorm);
    }
    return Number(e.weeklyNorm) || weeklyNorm;
  };
    ...
    for (const sh of smeneZaDan(config, d)) {
      ...
      const kandidati = employees
        ...
        .map((e) => {
          ...
          const manjka = norma == null ? 0 : norma - stanje[e.id].ur;
          ...
          // Zaposlena ima prednost, dokler ji do norme manjka vsaj pol smene;
          // zadnjih par ur ne pobira na račun študentk.
