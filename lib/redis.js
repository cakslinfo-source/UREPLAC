...
/** Doda element na konec seznama (atomarno - sporočila se ne povozijo). */
export async function rpush(key, value) {
  return cmd(['RPUSH', key, typeof value === 'string' ? value : JSON.stringify(value)]);
}

/** Prebere seznam; privzeto celega. */
export async function lrange(key, start = 0, stop = -1) {
  const res = await cmd(['LRANGE', key, String(start), String(stop)]);
  return Array.isArray(res) ? res : [];
}

/** Obdrži samo zadnjih n elementov. */
export async function ltrim(key, n) {
  return cmd(['LTRIM', key, String(-n), '-1']);
}
